...
async def cmd_report(message: types.Message) -> None:
    async with async_session() as session:
        summary = await crud.get_expenses_summary(session)
        total = await crud.get_total_expenses(session)
        foreman_balance = await crud.get_foreman_balance(session)
    ...
    for cat_name, cat_total in summary:
        lines.append(f"• *{cat_name}*: {cat_total:,.2f} сум")

    lines.append(f"\n💰 *Итого расходов:* {total:,.2f} сум")
    lines.append(f"\n👷 *Прорабу выдано:* {foreman_balance['total_given']:,.2f} сум")
    lines.append(f"👷 *Прораб отчитался:* {foreman_balance['total_settled']:,.2f} сум")
    lines.append(f"👷 *Неотчитанный остаток:* {foreman_balance['outstanding']:,.2f} сум")
...
async def cmd_categories(message: types.Message) -> None:
    async with async_session() as session:
        cats = await crud.get_all_categories(session)
...
async def cmd_foreman(message: types.Message) -> None:
    async with async_session() as session:
        balance = await crud.get_foreman_balance(session)
        transactions = await crud.get_all_foreman_transactions(session)

    lines = [
        "👷 *Баланс прораба*\n",
        f"Выдано всего: {balance['total_given']:,.2f} сум",
        f"Отчитался: {balance['total_settled']:,.2f} сум",
        f"Неотчитанный остаток: {balance['outstanding']:,.2f} сум",
    ]
...
async def settle_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    await callback.answer("Эта кнопка больше не используется. Используйте /settle", show_alert=True)
...
async def settle_description(message: types.Message, state: FSMContext) -> None:
    ...
    settling_tx_id = data.get("settling_tx_id")
    await state.clear()
    ...
    async with async_session() as session:
        ...
        tx = await crud.settle_foreman_transaction(
            session,
            tx_id=settling_tx_id,
            category_id=cat.id,
            settled_description=parsed.description or text,
        )
        await session.commit()

        if tx:
            await message.answer(
                f"✅ Выдача #{tx.id} закрыта!\n"
                f"Категория: *{cat.name}*\n"
                f"Сумма: {tx.amount:,.2f} сум",
            )
        else:
            await message.answer("⚠️ Не удалось найти или выдача уже закрыта.")
...
async def handle_message(message: types.Message) -> None:
    ...
    async with async_session() as session:
        ...
        if parsed.type == "expense":
            ...
            await message.answer(
                f"💰 Записано: выдано прорабу *{tx.amount:,.2f} сум*\n"
            ...
            await message.answer(
                ...
                f"Категория: *{cat.name}*\n"
                f"Сумма: *{expense.amount:,.2f} сум*\n"
                f"Остаток у прораба: *{balance['outstanding']:,.2f} сум*",
            )
